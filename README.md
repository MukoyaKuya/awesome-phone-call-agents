...
Recipients may be in any CALL-E-supported country and must use international E.164 format, such as `+12025550123` or `+254700000001`. By default CALL-E infers routing from the number. Set `MEDROUTE_CALL_LOCALE` and `MEDROUTE_CALL_REGION` only when CALL-E has confirmed that exact supported combination for the deployment. The server places the selected pharmacy calls sequentially by default, so it also works with accounts limited to one active call. The server requires both consent acknowledgements and a stable `Idempotency-Key` header before it will place a live call. The browser creates this key for each deliberate live submission; integrations must retain it when retrying the same request.
